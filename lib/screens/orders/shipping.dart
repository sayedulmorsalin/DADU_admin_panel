...
class _ShippingState extends State<Shipping> {
  ...
  Future<void> _shippedOrder(
    ...
    try {
      ...
      // Calculate COD amount using subtotal (fallback to total or calculated items sum)
      num rawSubtotal = _safeNum(order['subtotal'] ?? order['subTotal'] ?? order['sub_total']);
      if (rawSubtotal == 0) {
        rawSubtotal = _safeNum(order['total'] ?? order['totalAmount'] ?? order['total_price']);
      }
      if (rawSubtotal == 0) {
        final itemsList = getItems(order);
        for (final item in itemsList) {
          if (item is Map<String, dynamic>) {
            num price = _safeNum(item['price']);
            num qty = _safeNum(item['quantity']);
            if (qty <= 0) qty = 1;
            rawSubtotal += (price * qty);
          }
        }
      }
      double codAmount = rawSubtotal.toDouble();

      // Generate a unique alphanumeric invoice according to Steadfast requirements
      String rawInvoice = order['order_id']?.toString() ?? '';
      String invoice = rawInvoice.replaceAll(RegExp(r'[^a-zA-Z0-9_-]'), '');
      if (invoice.isEmpty) {
      ...
      String recipientName =
          (order['customerName'] ?? order['user_name'] ?? 'Customer')
              .toString()
              .trim();
      if (recipientName.isEmpty) recipientName = 'Customer';
